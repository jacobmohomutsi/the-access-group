...
export default function CTA({ ctaData }) {
  ...
  return (
    <section id="cta" className="bg-white text-primary h-auto md:h-screen flex items-center flex-col justify-center py-6">
      <div className="mx-auto max-w-8xl px-4 py-8 sm:px-6 lg:px-8 lg:py-20">
        <div className="flex flex-col gap-8 rounded-[2rem] bg-primary/5 p-6 backdrop-blur md:flex-row md:items-center md:justify-between">
          <div className="max-w-2xl">
            <p className="text-sm font-semibold uppercase tracking-[0.28em] text-primary/70">PRODUCTISED SERVICES FOR GROWTH</p>
            <h2 className="mt-3 text-3xl font-black tracking-tight sm:text-5xl">Your strategic gateway to market access.</h2>
      ...
      <div className="opacity-0 w-full px-4 animate-fade-up animation-delay-600 flex flex-col justify-center gap-4 sm:flex-row sm:flex-wrap">
        <a
          href="#partnerships"
          className="inline-flex w-full items-center justify-center rounded-2xl border border-white/10 bg-[#263B38] px-7 py-4 text-base font-semibold text-white hover:bg-white/10"
        >
          LinkedIn
        </a>
        <button
