...
export default function SuccessCalculator() {
  ...
  const projectSteps = [
    { id: 1, title: 'Strategic Intake', description: 'We define the scope, identify key constraints, and align on success metrics.', locked: false, button: 'Book Ph.D.-Led Diagnostic' },
    ...
    <section id="calculator" className="bg-primary text-white py-20 lg:py-24 border-t border-b border-white/5 relative overflow-hidden">
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          {tracks.map((track) => {
            ...
            return (
              <button
                ...
                className={`flex items-center gap-2.5 px-5 py-3.5 rounded-2xl font-bold text-sm transition-all duration-300 shadow-sm border ${
                  isActive 
        ...
        <div className="w-full max-w-7xl mx-auto grid grid-cols-1 xl:grid-cols-2 gap-6 sm:gap-8 items-start">
          ...
          <div className="bg-white/[0.03] border border-secondary/20 rounded-[2rem] p-8 sm:p-12 shadow-2xl transition-all duration-300 min-h-[400px] flex flex-col justify-center h-full">
          ...
          <div className="bg-white/[0.03] border border-secondary/20 rounded-[2rem] p-8 sm:p-12 shadow-2xl flex flex-col h-full lg:min-h-[600px] justify-between">
            <div className="flex-1 flex flex-col justify-center">
              <p className="text-sm text-white/60 text-center mb-10 font-medium px-4 animate-fade-in">
                {trackSubtitles[activeTrack]}
              </p>
              
              <div className="flex flex-col items-center animate-fade-in relative z-10 w-full max-w-sm mx-auto">
                {/* Icon Circle */}
                <div className={`w-16 h-16 rounded-full border-2 flex items-center justify-center bg-primary mb-6 ${
                  !projectSteps[currentStepIndex].locked ? 'border-secondary text-secondary shadow-[0_0_20px_rgba(194,166,107,0.3)]' : 'border-white/20 text-white/40'
                }`}>
                  {!projectSteps[currentStepIndex].locked ? <Check size={28} strokeWidth={3} /> : <Lock size={24} />}
                </div>

                {/* Content Box */}
                <div className={`w-full border rounded-3xl p-8 text-center transition-all duration-300 ${
                  ...
                  <div className="flex items-center justify-center gap-3 mb-4">
                    <span className={`text-sm font-bold uppercase tracking-widest ${
                      !projectSteps[currentStepIndex].locked ? 'text-secondary' : 'text-white/40'
                    }`}>
                  ...
                  <h4 className={`text-2xl font-black tracking-tight mb-4 ${
                    !projectSteps[currentStepIndex].locked ? 'text-white' : 'text-white/80'
                  }`}>
                    {projectSteps[currentStepIndex].title}
                  </h4>
                  
                  <p className={`text-base leading-relaxed ${
                    !projectSteps[currentStepIndex].locked ? 'text-white/80' : 'text-white/50'
                  }`}>
            ...
            <div className="mt-12 flex items-center justify-center gap-6">
              <button 
                ...
                className={`w-12 h-12 rounded-full border border-white/20 flex items-center justify-center transition-colors ${currentStepIndex === 0 ? 'opacity-30 cursor-not-allowed' : 'hover:bg-white/10 text-white'}`}
              >
                <ChevronLeft size={20} />
              </button>
                  ...
                  <div 
                    ...
                    className={`transition-all duration-300 rounded-full ${
                      idx === currentStepIndex 
                        ...
                        : 'w-2.5 h-2.5 bg-white/20 hover:bg-white/40 cursor-pointer'
                    }`}
              ...
              <button 
                ...
                className={`w-12 h-12 rounded-full border border-white/20 flex items-center justify-center transition-colors ${currentStepIndex === projectSteps.length - 1 ? 'opacity-30 cursor-not-allowed' : 'hover:bg-white/10 text-white'}`}
              >
